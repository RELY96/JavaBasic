...
//** Date - 날짜 처리 클래스 : 컴퓨터의 현재 날짜.시간를 읽어 Date 객체를 생성
//=> 비권장( 더이상 지원하지 않음 )  
//   Deprecated :중요도가 떨어져 "더 이상 사용되지 않고 앞으로는 사라지게 될" (컴퓨터 시스템 기능 등)

//==================================================================================//
// 클래스별로 폼을 지원해주는 클래스 중 Date Type의 날짜 시간을 폼 정해주는 클래스
//** SimpleDateFormat
...
public class Ex01_Date {
	...
	public static void main(String[] args) {
		...
		Date now = new Date() ; //Date 객체 생성 , 후에 내장된 매서드 이용
		System.out.println("** now => "+now);
		System.out.println("** Year => "+(now.getYear()+1900));  // +1900
		System.out.println("** Month => "+(now.getMonth()+1));   // +1   *Month는 출력 시 늘 +1 해줘야함!
		System.out.println("** Date => "+now.getDate()+","+now.getDay());
		System.out.printf("** Time => %d:%d:%d \n",now.getHours(),now.getMinutes(),now.getSeconds());
		
		// ** SimpleDateFormat
		SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd") ; //SimpleDateFormat 클래스 sf의 매개변수를 이용해 폼 잡기 -> 매개변수now를 폼에 맞게 적용해서 출력
