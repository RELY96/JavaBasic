...
//** Calendar : 추상 클래스(new 인스턴스 생성 불가)
//   추상클래스로 만든 이유: 날짜, 시간 계산법이 지역과 문화에 따라 다르기 때문에 꼭 필요한 메서드만 정의해서, 특정한 역법을 따르는 경우엔 하위 클래스에서 구현 하도록 정의한 것
//   Calendar클래스 안 getInstance()라는 매서드를 통해 "운영체제가 적용"하는 날짜, 시간값을 사용하는 것 
...
public class Ex02_Calendar {
	...
	public static void main(String[] args) {
		...
		System.out.println("Calendar.YEAR => "+Calendar.YEAR); //Calendar.YEAR는 운영체제 내부적으로 갖고 있는 숫자값 => getInstance()안에 넣어서 출력해줘야 함!
		...
		// ** Formatter 활용 : Calendar클래스의 폼 잡아주는 클래스 - Formatter는 Calendar뿐 아니라 다른 클래스의 폼을 잡아줄 수 있다!
		...
		f.close(); //Formatter 는 close 해줘야 함
